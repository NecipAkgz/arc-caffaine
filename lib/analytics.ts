...
export function getAverageDonation(memos: Memo[]): number {
  ...
  // Sum bigint natively first, then format once at the end.
  // This avoids O(N) string and float allocations inside the loop.
...
export function getTopSupporters(
  ...
  // Sum bigint natively per user to avoid O(N) formatting and float allocations inside the loop
