...
export function getAverageDonation(memos: Memo[]): number {
  ...
  // Summing native bigints first avoids O(N) string/float allocations
  // before formatting
  let totalBigInt = 0n;
  for (let i = 0; i < memos.length; i++) {
...
export function getTopSupporters(
  ...
  for (let i = 0; i < memos.length; i++) {
    ...
    const existing = supporterMap.get(address) || { totalAmountBigInt: 0n, count: 0 };
    supporterMap.set(address, {
      totalAmountBigInt: existing.totalAmountBigInt + memo.amount,
