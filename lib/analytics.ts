...
export function getAverageDonation(memos: Memo[]): number {
  ...
  // Optimize by aggregating BigInt amounts directly before converting to string/float.
  // Avoids O(N) formatEther string allocations and parseFloat calls.
  const totalBigInt = memos.reduce(
    (sum, memo) => sum + BigInt(memo.amount.toString()), // handle possible string/number from on-chain via BigInt
    BigInt(0),
  );
...
export function getTopSupporters(
  ...
  // Optimize by aggregating BigInt natively
  ...
  for (const memo of memos) {
    ...
    const amountBigInt = BigInt(memo.amount.toString());
    const existing = supporterMap.get(address) || { totalAmountBigInt: BigInt(0), count: 0 };
    supporterMap.set(address, {
      totalAmountBigInt: existing.totalAmountBigInt + amountBigInt,
